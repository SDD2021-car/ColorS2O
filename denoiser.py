...
class Denoiser(nn.Module):
    ...
    def forward(self, opt_img, sar_img, labels=None, epoch_lambda=1.0):
        if labels is None:
        ...
        x_pred = self.net(x_in, t.flatten(), labels_dropped)
        ...
        if "ssim" in self.enabled_losses:
            if self.no_t_schedule:
                ...
                lambda_ssim = self._lambda_color2(
                    t_flat, self.lambda_ssim_max, self.lambda_ssim_final, self.lambda_ssim_t0, self.lambda_ssim_t1, self.lambda_ssim_alpha, self.lambda_ssim_beta
                )
    ...
    def _forward_sample(self, z, t, labels, sar_img):
        # conditional
        x_in = self._concat_sar(z, sar_img)
        x_cond = self.net(x_in, t.flatten(), labels)
        v_cond = (x_cond - z) / (1.0 - t).clamp_min(self.t_eps)

        low, high = self.cfg_interval
        if self.cfg_scale == 1.0 or high <= low or high <= 0.0 or low >= 1.0:
            return v_cond

        # unconditional
        x_uncond = self.net(x_in, t.flatten(), torch.full_like(labels, self.num_classes))
