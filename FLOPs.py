...
Example:
    python FLOPs.py --model JiT-B/8 --img_size 512 --batch_size 1
"""
...
def _count_flops_fvcore(
    ...
    with torch.no_grad():
        analysis = FlopCountAnalysis(model, (x, t, y))
...
def main() -> None:
    ...
    flops = _count_flops_torch_mode(model, x, t, y)
    backend = "torch.utils.flop_counter"
    if flops is None:
        flops = _count_flops_fvcore(model, x, t, y)
        backend = "fvcore"
