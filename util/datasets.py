...
def _randint(max_value):
    return torch.randint(0, max_value, (1,)).item()
...
def build_hints(
    ...
    if torch.rand(1).item() < hint_dropout_prob:
    ...
    while hint_count < max_pixels and attempts < max_attempts:
        attempts += 1
        seed_y = _randint(height)
        seed_x = _randint(width)

        if hint_sampling_mode == "stripe":
            theta = _uniform(0.0, math.pi)
            thickness = _uniform(1.0, 4.0)
            length = _uniform(5.0, 30.0)
